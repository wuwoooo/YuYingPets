...
} from "../components/ScoreRecordReverseModal";
...
import {
  canShowClassScoreRecordReverse,
  canShowScoreRecordReverse,
  formatScoreDelta,
...
function formatClassRuleGroupScore(group: {
  add?: ScoreRule;
  deduct?: ScoreRule;
}) {
  const parts: string[] = [];
...
export function EvaluationPage({
  ...
  const rolePresentation = roleTitleMap[user?.roleCode ?? ""] ?? {
    title: "学生评价",
    subtitle: "对学生进行规则化评价，并回看最近记录。",
  };
  const canManageClassScore = [
    "super_admin",
    "school_admin",
    "academic_admin",
    "moral_admin",
  ...
  const [classBatchSelectionEditing, setClassBatchSelectionEditing] =
    useState(false);
  ...
  const [batchSelectionEditing, setBatchSelectionEditing] = useState(false);
  const [selectedGroupId, setSelectedGroupId] = useState<number | null>(null);
  const [subjectFilter, setSubjectFilter] = useState<string>("all");
  const [studentKeyword, setStudentKeyword] = useState("");
  const [studentSort, setStudentSort] = useState<StudentSortKey>("name-asc");
  const [groupFilter, setGroupFilter] = useState<number | "all">("all");
  const [scoreModalTarget, setScoreModalTarget] = useState<ScoreTarget | null>(
    null,
  );
  ...
  const [recentSubmitRecords, setRecentSubmitRecords] = useState<
    RecentSubmitRecord[]
  >([]);
  const [reverseTarget, setReverseTarget] = useState<ReverseTarget | null>(
    null,
  );
  const [classReverseTarget, setClassReverseTarget] =
    useState<ClassReverseTarget | null>(null);
  const [reverseLoading, setReverseLoading] = useState(false);
  const today = new Date().toISOString().slice(0, 10);
  const summaryDefaultStartDate = new Date(Date.now() - 6 * 24 * 60 * 60 * 1000)
    .toISOString()
    .slice(0, 10);
  const [summaryStartDate, setSummaryStartDate] = useState(
    summaryDefaultStartDate,
  );
  ...
  const [classScoreDateInitialized, setClassScoreDateInitialized] =
    useState(false);
  const [classRankingRows, setClassRankingRows] = useState<
    ClassScoreRankingRow[]
  >([]);
  const [classRankingLoading, setClassRankingLoading] = useState(false);
  const [classRankingError, setClassRankingError] = useState<string | null>(
    null,
  );
  ...
  const [scoreSummaryError, setScoreSummaryError] = useState<string | null>(
    null,
  );
  const [scoreSummaryRows, setScoreSummaryRows] = useState<
    NonNullable<AnalyticsData["scoreDetailSummary"]>
  >([]);
  const [groupScoreManageOpen, setGroupScoreManageOpen] = useState(false);
  const [groupScoreManageTargetId, setGroupScoreManageTargetId] = useState<
    number | null
  >(null);
  const [groupScoreRankingRows, setGroupScoreRankingRows] = useState<
    GroupScoreRankingRow[]
  >([]);
  const [groupScoreRecords, setGroupScoreRecords] = useState<
    GroupScoreRecordRow[]
  >([]);
  const [groupScoreRecordsLoading, setGroupScoreRecordsLoading] =
    useState(false);
  const [groupScoreRecordsError, setGroupScoreRecordsError] = useState<
    string | null
  >(null);
  ...
  const [groupScoreRecordFilter, setGroupScoreRecordFilter] = useState<
    "current" | "all"
  >("current");
  ...
  const homeroomClassIds = useMemo(
    () =>
      availableClasses
        .filter((item) => item.homeroomTeacher?.id === user?.id)
        .map((item) => item.id),
    ...
    if (recordKeyword.trim()) {
      ...
      rows = rows.filter((item) => {
        const student = students.find(
          (studentRow) => studentRow.id === item.studentId,
        );
  ...
  const sortedOverviewGroups = useMemo(
    () =>
      [...groups].sort(
        (left, right) =>
          (groupRankMap.get(left.id) ?? Number.MAX_SAFE_INTEGER) -
            (groupRankMap.get(right.id) ?? Number.MAX_SAFE_INTEGER) ||
          (right.groupScore ?? 0) - (left.groupScore ?? 0) ||
          right.studentCount - left.studentCount ||
          left.groupNo - right.groupNo,
      ),
  ...
  const normalizedScoreSummaryRows = useMemo(() => {
    const summaryMap = new Map(
      scoreSummaryRows.map((item) => [item.studentId, item]),
    );
  ...
  const filteredScoreSummaryRows = useMemo(() => {
    ...
    return normalizedScoreSummaryRows.filter((item) =>
      matchPinyinOrChinese(item.studentName, keyword),
    );
  ...
  const scoreRecordQuery = useMemo(
    () => ({
      ...
      ...(recordStudentFilter !== "all"
        ? { studentId: recordStudentFilter }
        : {}),
      ...(isSubjectTeacher && activeSubjectView?.subjectCode
  ...
  const classScoreRecordQuery = useMemo(() => {
    const isBatch = isClassEvaluationPage && classMode === "batch";
    const batchGradeClassIds = isBatch
      ? availableClasses
          .filter((item) => item.gradeName === classBatchGradeName)
          .map((item) => item.id)
      : undefined;

    return {
      classId: isBatch ? undefined : (selectedClassId ?? undefined),
      classIds: isBatch ? batchGradeClassIds : undefined,
  ...
  const classScoreDateMax =
    semesterEndDate && semesterEndDate > today ? semesterEndDate : today;
  const canManageGroupScore = [
    "super_admin",
    "school_admin",
    "academic_admin",
    "homeroom_teacher",
  ...
  const activeGroupScoreRow = useMemo(
    () =>
      groupScoreRankingRows.find(
        (item) => item.id === groupScoreManageTargetId,
      ) ?? null,
  ...
  const evaluationRules = useEvaluationRules({
    ...
    initialScoreTypeFilter: isClassEvaluationPage ? "all" : "quick",
  });
  ...
  const reloadStudentHonorRecords = useCallback(
    ...
    setStudentScoreOverrides((prev) => {
      ...
      items.forEach((item) => {
        if (item.studentProfile.studentId != null) {
          next[item.studentProfile.studentId] =
            item.studentProfile.currentScore;
        }
      });
      return next;
    });
  ...
  const availableSubjectFilters = useMemo(() => {
    const currentSubjectCodes = selectedClassId
      ? (subjectCodesByClass.get(selectedClassId) ?? [])
      : [];
  ...
  const availableClassRules = useMemo(() => {
    const classRules = rules.filter((item) => {
      ...
      if (scoreTypeFilter !== "all" && item.scoreType !== scoreTypeFilter)
        return false;
      if (ruleKeyword.trim()) {
  ...
  const activeRules = isClassEvaluationPage
    ? availableClassRules
    : availableRules;
  ...
  const averageScore = classStudents.length
    ? Math.round(
        classStudents.reduce((sum, item) => sum + item.currentScore, 0) /
  ...
  useEffect(() => {
    ...
    if (
      querySubjectCode &&
      availableSubjectFilters.includes(querySubjectCode)
    ) {
      if (!isSubjectTeacher) setSubjectFilter(querySubjectCode);
    } else if (
      !querySubjectCode &&
      subjectFilter !== "all" &&
      !availableSubjectFilters.includes(subjectFilter)
    ) {
      setSubjectFilter("all");
    }
  }, [
    availableClasses,
    availableSubjectFilters,
    isSubjectTeacher,
    searchParams,
    subjectFilter,
  ]);
  ...
  useEffect(() => {
    ...
    if (
      parsedStudentId &&
      classStudents.some((item) => item.id === parsedStudentId)
    ) {
    ...
    setSelectedStudentIds((prev) =>
      prev.filter((item) =>
        classStudents.some((student) => student.id === item),
      ),
    );
  }, [
    classStudents,
  ...
  useEffect(() => {
    ...
    setRecordStudentFilter(
      mode === "single" && selectedStudentId ? selectedStudentId : "all",
    );
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      canManageClassScore
        ? adminApi.classScoreRecords(token, classScoreRecordQuery)
        : Promise.resolve({ data: [] as ClassScoreRecord[] }),
    ])
  ...
  useEffect(() => {
    ...
    const request =
      isSubjectTeacher && activeSubjectView
        ? adminApi
            ...
            .then(
              (response) =>
                response.data
                  .scoreDetailSummary as TeacherReviewContext["scoreDetailSummary"],
            )
        : adminApi
    ...
    request
      ...
      .catch((err) => {
        ...
        setScoreSummaryError(
          err instanceof Error ? err.message : "评分明细汇总加载失败",
        );
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    groupScoreManageOpen,
    groupScoreManageTargetId,
    groupScoreRecordFilter,
    selectedClassId,
  ]);
  ...
  async function handleGroupScoreAdjustSubmit() {
    ...
    try {
      ...
      setGroupScoreRecordsError(
        err instanceof Error ? err.message : "调整小组积分失败",
      );
    } finally {
  ...
  async function handleResetAllGroupScores() {
    ...
    const hasNonZero = groupScoreRankingRows.some(
      (item) => item.groupScore !== 0,
    );
    if (!hasNonZero) {
    ...
    setGroupScoreRecordsError(null);
    try {
      await adminApi.resetGroupScores(token, selectedClassId, {
        sourceTerminal: "admin",
      });
      await refreshGroupScoreArtifacts(groupScoreManageTargetId);
      ...
      setGroupScoreRecordsError(
        err instanceof Error ? err.message : "小组积分清零失败",
      );
    } finally {
  ...
  function getSelectionError() {
    ...
    if (isClassEvaluationPage) {
      ...
      if (classMode === "batch" && selectedClassScoreIds.length === 0)
        return "请至少选择一个班级";
      return null;
    }
    if (mode === "single" && !selectedStudentId) return "请先选择学生";
    if (mode === "batch" && selectedStudentIds.length === 0)
      return "请至少选择一名学生";
  ...
  async function reloadCurrentClassData() {
    if (!selectedClassId) return;
    const [recordResponse, groupResponse, classRecordResponse] =
      await Promise.all([
        adminApi.scoreRecords(token, scoreRecordQuery),
        adminApi.classGroups(token, selectedClassId),
        canManageClassScore
  ...
  async function loadGroupScoreArtifacts(
    classId: number,
    ...
    try {
      const targetGroupId =
        groupScoreRecordFilter === "all"
          ? null
          : (focusGroupId ?? groupScoreManageTargetId);
      await loadGroupScoreArtifacts(selectedClassId, targetGroupId);
      await reloadCurrentClassData();
    } catch (err) {
      setGroupScoreRecordsError(
        err instanceof Error ? err.message : "小组积分数据加载失败",
      );
    } finally {
  ...
  async function handleReverseConfirm(remark: string) {
    ...
    setSubmitError(null);
    try {
      const response = await adminApi.reverseScoreRecord(
        token,
        reverseTarget.record.id,
        { remark },
      );
      await Promise.all([onSaved(), reloadCurrentClassData()]);
      setRecentSubmitRecords((prev) =>
        prev.filter((item) => item.scoreRecordId !== reverseTarget.record.id),
      );
  ...
  function openClassReverseModal(record: ClassScoreRecord) {
    ...
    setClassReverseTarget({
      ...
      currentScore:
        targetClass?.classScore ?? selectedClass?.classScore ?? null,
    });
  }
  ...
  async function handleClassReverseConfirm(remark: string) {
    ...
    setSubmitError(null);
    try {
      const response = await adminApi.reverseClassScoreRecord(
        token,
        classReverseTarget.record.id,
        { remark },
      );
      await Promise.all([onSaved(), reloadCurrentClassData()]);
      if (rankGradeCode) {
  ...
  function buildRecentSubmitRecordsFromBatch(
    ...
    return items
      ...
      .map((item) => {
        ...
        const student =
          classStudents.find((row) => row.id === studentId) ??
          students.find((row) => row.id === studentId);
        return {
      ...
      .filter(
        (item) =>
          studentIds.length === 0 || studentIds.includes(item.studentId),
      );
  }
  ...
  async function submitEvaluation(rule: ScoreRule, remark: string) {
    ...
    try {
      ...
      if (mode === "batch") {
        ...
        setRecentSubmitRecords(
          buildRecentSubmitRecordsFromBatch(
            response.data.items,
            rule,
            selectedStudentIds,
          ),
      ...
      if (mode === "group") {
        ...
        setRecentSubmitRecords(
          buildRecentSubmitRecordsFromBatch(response.data.items, rule, []),
        );
        submitBatchItems = response.data.items;
      }
  ...
  async function submitClassEvaluation(rule: ScoreRule, remark: string) {
    ...
    try {
      ...
      if (classMode === "single") {
        ...
        if (selectedClassScoreIds.length === 0)
      ...
      setConfirmRemark("");
      setSubmitSuccess(
        classMode === "batch" ? "批量班级评价已提交" : "班级评价已提交",
      );
    } catch (err) {
  ...
  const classRuleCards = useMemo(
    () => availableClassRules.slice(0, 36),
    [availableClassRules],
  );
  const groupedClassRuleCards = useMemo(() => {
    const groups = new Map<
      string,
      { metric: string; add?: ScoreRule; deduct?: ScoreRule }
    >();
  ...
  const gradeOptions = useMemo(
    () =>
      Array.from(new Set(availableClasses.map((item) => item.gradeName))).sort(
        compareGradeName,
      ),
    [availableClasses],
  );
  ...
  const classesSorted = useMemo(
    () =>
      [...availableClasses].sort(
        (left, right) =>
          compareGradeName(left.gradeName, right.gradeName) ||
  ...
  useEffect(() => {
    if (!gradeOptions.includes(classSingleGradeName)) {
      setClassSingleGradeName(
        selectedClass?.gradeName ?? gradeOptions[0] ?? "",
      );
    }
  ...
  useEffect(() => {
    ...
    setSelectedClassScoreIds((prev) =>
      prev.filter((item) => allowed.has(item)),
    );
  }, [batchGradeClasses, classMode]);

  const rankGradeCode =
    classesSorted.find((item) => item.gradeName === rankGradeName)?.gradeCode ??
    "";
  ...
  useEffect(() => {
    ...
    adminApi
      ...
      .catch((err) => {
        if (!active) return;
        setClassRankingRows([]);
        setClassRankingError(
          err instanceof Error ? err.message : "班级积分排名加载失败",
        );
      })
      .finally(() => {
  ...
  const selectionSummary = useMemo(() => {
    ...
    return {
      title: group
        ? `小组评价 · 第${group.groupNo}组 ${group.name}`
        : "小组评价",
      subtitle: group
        ? `${group.studentCount} 人 · 当前 ${group.currentScoreTotal} 分`
        : "请先选择评价小组",
    };
  }, [
    classStudents,
  ...
  return (
    <Shell
      ...
      status={
        <>
          {loading || pageLoading ? (
            <div className="status-card">评价数据加载中...</div>
          ) : null}
          ...
          {!isClassEvaluationPage && recentSubmitRecords.length > 0 ? (
            <div className="status-card evaluation-recent-reverse-card">
              <div className="evaluation-recent-reverse-head">
                <strong>
                  {recentSubmitRecords.length === 1
                    ? "本次评价已提交"
                    : `本次已为 ${recentSubmitRecords.length} 名学生评价`}
                </strong>
                <button
                  type="button"
                  className="score-record-link-button"
                  onClick={() => setRecentSubmitRecords([])}
                >
                  关闭
                </button>
              </div>
              <div className="evaluation-recent-reverse-list">
                {recentSubmitRecords.map((item) => (
                  <div
                    className="evaluation-recent-reverse-item"
                    key={item.scoreRecordId}
                  >
                    <span>
                      {item.studentName} ·{" "}
                      {formatScoreRecordLabel({
    ...
    >
      <div className="admin-list-desk evaluation-desk">
        <div className="page-header admin-list-page-header">
          <div>
            <h2>
              {isClassEvaluationPage ? "班级评价" : rolePresentation.title}
            </h2>
              ...
              <>
                <select
                  ...
                  onChange={(event) => {
                    ...
                    const nextClass = classesSorted.find(
                      (item) => item.gradeName === nextGrade,
                    );
        ...
        !(isClassEvaluationPage && classMode === "batch") ? (
          <div
            className={`std-metric-grid${isClassEvaluationPage ? " std-metric-grid--4" : " std-metric-grid--3"}`}
          >
            <div className="std-metric-card std-metric-card--blue">
              <div className="std-metric-card__top">
                <div className="std-metric-card__icon">
                  <span className="sec-metric-glyph">班</span>
                </div>
              ...
              <div className="std-metric-card__hint">
                {selectedClass?.slogan ??
                  "班级口号待补充，可由班主任在「我的班级」中维护。"}
              </div>
            </div>
            <div className="std-metric-card std-metric-card--green">
              <div className="std-metric-card__top">
                <div className="std-metric-card__icon">
                  <span className="sec-metric-glyph">生</span>
                </div>
            ...
            {isClassEvaluationPage ? (
              <div className="std-metric-card std-metric-card--purple">
                <div className="std-metric-card__top">
                  <div className="std-metric-card__icon">
                    <span className="sec-metric-glyph">分</span>
                  </div>
                  <span className="std-metric-card__label">班级积分</span>
                </div>
                <div className="std-metric-card__value">
                  {selectedClass?.classScore ?? 0}
                </div>
                <div className="std-metric-card__hint">
                  独立于学生个人积分，由班级评价调整
                </div>
            ...
            <div
              className={`std-metric-card std-metric-card--amber${!isClassEvaluationPage ? "" : ""}`}
            >
              <div className="std-metric-card__top">
                <div className="std-metric-card__icon">
                  <span className="sec-metric-glyph">记</span>
                </div>
        ...
        <div className="evaluation-layout">
          <div className="panel evaluation-form-panel admin-list-panel">
            <div className="panel-title">
              {isClassEvaluationPage ? "发起班级评价" : "发起学生评价"}
            </div>
              ...
              <>
                <div className="evaluation-mode-grid">
                  <button
                    ...
                    onClick={() => {
                      setClassMode("single");
                      setSelectedClassScoreIds([]);
                      setClassBatchSelectionEditing(false);
                    }}
                  >
                    <strong>单班评价</strong>
                      ...
                      <div className="class-eval-selector-stack">
                        <label className="class-eval-selector-row">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              setClassSingleGradeName(event.target.value)
                            }
                          >
                            {gradeOptions.map((item) => (
                              <option key={item} value={item}>
                        ...
                        <p>
                          {selectedClass
                            ? `${selectedClass.gradeName} ${selectedClass.name} · 当前 ${selectedClass.classScore} 分`
                            : "请先选择班级"}
                        </p>
                      </div>
                    ) : (
                      ...
                      >
                        ...
                        >
                          <select
                            ...
                            onChange={(event) =>
                              setClassBatchGradeName(event.target.value)
                            }
                          ...
                          {batchGradeClasses.length > 0 ? (
                            <button
                              ...
                              onClick={() =>
                                setClassBatchSelectionEditing((prev) => !prev)
                              }
                            >
                              {classBatchSelectionEditing ? "完成" : "编辑"}
                            </button>
                          ) : null}
                          {classBatchSelectionEditing &&
                          batchGradeClasses.length > 0 ? (
                            <button
                              type="button"
                              className="score-record-reverse-button"
                              ...
                              onClick={() => {
                                const isAllSelected = batchGradeClasses.every(
                                  (item) =>
                                    selectedClassScoreIds.includes(item.id),
                                );
                                if (isAllSelected) {
                                  setSelectedClassScoreIds((prev) =>
                                    prev.filter(
                                      (id) =>
                                        !batchGradeClasses.some(
                                          (c) => c.id === id,
                                        ),
                                    ),
                                  );
                                  ...
                                  setSelectedClassScoreIds((prev) => {
                                    ...
                                    return [
                                      ...otherIds,
                                      ...batchGradeClasses.map((c) => c.id),
                                    ];
                                  });
                                }
                              }}
                            >
                              {batchGradeClasses.every((item) =>
                                selectedClassScoreIds.includes(item.id),
                              )
                                ? "取消全选"
                                : "全选"}
                        ...
                        <div
                          className="evaluation-student-grid"
                          style={{ flex: 1 }}
                        >
                          {batchGradeClasses.map((item) => (
                            <label
                              key={item.id}
                              className={`evaluation-student-card${selectedClassScoreIds.includes(item.id) ? " active" : ""}`}
                            >
                              {classBatchSelectionEditing ? (
                                <input
                                  type="checkbox"
                                  checked={selectedClassScoreIds.includes(
                                    item.id,
                                  )}
                                  onChange={() =>
                                    toggleClassScoreClass(item.id)
                                  }
                                />
                              ) : null}
                              <span>
                    ...
                    <div className="evaluation-more-panel class-evaluation-rule-panel">
                      <div className="evaluation-rule-toolbar">
                        ...
                        <input
                          ...
                          value={ruleKeyword}
                          onChange={(event) =>
                            setRuleKeyword(event.target.value)
                          }
                      ...
                      <div className="rule-card-list compact">
                        {groupedClassRuleCards.map((group) => (
                          <div
                            key={group.metric}
                            className="class-rule-pair-card"
                          >
                            <div className="class-rule-pair-head">
                              <div className="class-rule-pair-title">
                                {group.metric}
                              </div>
                              <span className="class-rule-pair-score">
                                {formatClassRuleGroupScore(group)}
                              </span>
                            </div>
                        ...
                        {groupedClassRuleCards.length === 0 ? (
                          <p className="muted-text">暂无可用班级评价规则</p>
                        ) : null}
                      </div>
                    </div>
                  </div>
                ...
                <div className="detail-card class-evaluation-record-card">
                      ...
                      <p>
                        {classMode === "single"
                          ? selectedClass
                            ...
                            : "当前班级暂无记录"
                          : `${classBatchGradeName} ${classScoreStartDate && classScoreEndDate ? `${classScoreStartDate} 至 ${classScoreEndDate}` : "最近"} ${classScoreRecords.length} 条`}
                      </p>
                    </div>
                  </div>
                  <div className="class-evaluation-record-list">
                    {classScoreRecords.slice(0, 20).map((item) => (
                      <div
                        className="class-evaluation-record-item"
                      ...
                      >
                        <div className="class-evaluation-record-main">
                          <div className="class-evaluation-record-title">
                            ...
                            <span
                              className={`rule-select-score ${item.scoreDelta < 0 ? "deduct" : "add"}`}
                            >
                              {item.scoreDelta > 0 ? "+" : ""}
                              {item.scoreDelta}
                            </span>
                          ...
                          {item.reversedAt ? (
                            <div className="class-evaluation-record-reversed">
                              已撤销：{item.reverseRemark || "无撤销原因"} ·{" "}
                              {item.reversedByName || "管理员"} ·{" "}
                        ...
                        <div className="class-evaluation-record-meta">
                          <span>
                            {item.operatorName || item.sourceRole || "系统"}
                          </span>
                          <span>{formatDateTime(item.createdAt)}</span>
                          {!item.reversedAt &&
                          canShowClassScoreRecordReverse(item, user) ? (
                            <button
                              type="button"
                              className="score-record-reverse-button"
                              onClick={() => openClassReverseModal(item)}
                            >
                              撤销
                            </button>
                          ) : null}
                          {item.reversedAt ? (
                            <span className="score-record-reversed-tag">
                              已撤销
                            </span>
                          ) : null}
                        </div>
                ...
                <EvaluationToolbar
                  ...
                  onModeChange={(nextMode) => {
                    ...
                    if (nextMode !== "batch") {
                      ...
                      setBatchSelectionEditing(false);
                    }
                  }}
                  selectedGroupId={selectedGroupId}
                  onGroupIdChange={setSelectedGroupId}
                  selectedCount={selectedStudentIds.length}
                  selectionEditing={batchSelectionEditing}
                  onSelectionEditingChange={setBatchSelectionEditing}
                ...
                <div className="detail-card evaluation-score-summary-panel evaluation-score-summary-panel--main">
                  <div className="evaluation-score-summary-head">
                    <div>
                      ...
                      <div className="evaluation-score-summary-subtitle">
                        {scoreSummaryDescription}
                      </div>
                    </div>
                  </div>
                  <div className="evaluation-score-summary-toolbar">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setSummaryStudentKeyword(event.target.value)
                      }
                      placeholder="按学生姓名筛选"
                    />
                  </div>
                  ...
                  <div className="evaluation-score-summary-period-stats">
                    <span>
                      本周期 {scoreSummaryPeriodStats.recordCount} 条评价
                    </span>
                    <span>共加 {scoreSummaryPeriodStats.totalAddScore} 分</span>
                    <span>
                      共扣 {scoreSummaryPeriodStats.totalDeductScore} 分
                    </span>
                  </div>
                  {scoreSummaryError ? (
                    <div className="table-empty">{scoreSummaryError}</div>
                  ) : null}
                  <div className="evaluation-score-summary-list">
                    ...
                    {!scoreSummaryLoading &&
                      filteredScoreSummaryRows.map((item) => (
                        <button
                          type="button"
                          ...
                          onClick={() =>
                            applySummaryStudentToRecords(item.studentId)
                          }
                        >
                          <div className="evaluation-score-summary-card-head">
                            <div className="evaluation-score-summary-card-copy">
                              ...
                              <span>
                                {item.recordCount} 条（正向 {item.positiveCount}{" "}
                                / 负向 {item.negativeCount}）
                              </span>
                            </div>
                            <div
                              className={`evaluation-score-summary-total ${item.totalScoreDelta < 0 ? "deduct" : "add"}`}
                            >
                    ...
                    {!scoreSummaryLoading &&
                    !scoreSummaryError &&
                    filteredScoreSummaryRows.length === 0 ? (
                      <div className="evaluation-score-summary-empty">
                        {normalizedScoreSummaryRows.length === 0
                          ? "当前班级暂无可展示的学生。"
                          : "没有匹配的学生。"}
                      </div>
                    ) : null}
          ...
          <div className="evaluation-side-column">
                    ...
                    sortedOverviewGroups.map((item) => (
                      ...
                      >
                        <div className="evaluation-group-overview-item">
                          <strong className="evaluation-group-overview-name">
                            第 {groupRankMap.get(item.id) ?? "-"} 名 ·{" "}
                            {item.name}
                          </strong>
                          <span className="evaluation-group-overview-metric">
                            积分 {item.groupScore ?? 0} 分
                          </span>
                          <span className="evaluation-group-overview-metric">
                            人数 {item.studentCount} 人
                          </span>
                        </div>
                      </div>
                    ))
            ...
            {isClassEvaluationPage ? (
              <div className="panel admin-list-panel">
                ...
                {classRankingError ? (
                  <div className="table-empty">{classRankingError}</div>
                ) : null}
                <div className="mini-list">
                  {classRankingLoading ? (
                    <div className="table-empty">班级积分排名加载中...</div>
                  ) : null}
                  {!classRankingLoading &&
                    classRankRows.map((item) => (
                      <div
                        className="mini-list-item"
                        key={`class-rank-${item.id}`}
                      >
                        <div>
                          <strong>
                            第 {item.rank} 名 · {item.gradeName}{" "}
                            {item.className}
                          </strong>
                  ...
                  {!classRankingLoading && classRankRows.length === 0 ? (
                    <div className="table-empty">当前筛选条件下暂无班级。</div>
                  ) : null}
                </div>
              </div>
            ) : null}
                  ...
                  {displayedRecords.map((item) => {
                    ...
                    return (
                      <ScoreRecordListItem
                        ...
                        canReverse={canShowScoreRecordReverse(item, user, {
                          homeroomClassIds,
                        })}
          ...
          <StudentScoreModal
            ...
            onHonorGranted={async () => {
              ...
              if (scoreModalTarget?.type === "single" && selectedClassId) {
                await reloadStudentHonorRecords(
                  scoreModalTarget.studentId,
                  selectedClassId,
                );
              }
            }}
              ...
              <div className="group-score-manage-layout">
                <section className="group-score-manage-sidebar">
                  <div className="group-score-manage-section-head">
                    ...
                    <em>
                      {selectedClass
                        ? `${selectedClass.gradeName}${selectedClass.name} · ${groupScoreRankingRows.length} 组`
                        : `${groupScoreRankingRows.length} 组`}
                    </em>
                  </div>
                  ...
                  <div className="group-score-ranking-list">
                    {groupScoreRankingRows.length > 0 ? (
                      groupScoreRankingRows.map((row) => (
                        ...
                        >
                          <span className="group-score-ranking-rank">
                            {row.rank}
                          </span>
                          <span className="group-score-ranking-name">
                            {row.name}
                          </span>
                          <span className="group-score-ranking-score">
                            {formatSignedScore(row.groupScore)}
                          </span>
                        </button>
                      ))
                    ) : (
                      <div className="group-score-ranking-empty">
                        当前班级暂无小组。
                      </div>
                    )}
                  </div>
                ...
                <section className="group-score-manage-main">
                  <div className="group-score-manage-current">
                    <div className="group-score-manage-current-copy">
                      <span className="group-score-manage-current-label">
                        当前选中
                      </span>
                      <strong>
                        {activeGroupScoreRow?.name ?? "请选择小组"}
                      </strong>
                      <p>
                        {activeGroupScoreRow
                          ? `第${activeGroupScoreRow.rank}名 · 当前 ${activeGroupScoreRow.groupScore} 分 · 累计 ${activeGroupScoreRow.groupTotalScore} 分`
                          : "点击左侧小组后可调整积分并查看记录"}
                      </p>
                    </div>
                    {activeGroupScoreRow ? (
                      <div
                        className={`group-score-manage-current-score ${activeGroupScoreRow.groupScore < 0 ? "down" : "up"}`}
                      >
                        <span>实时积分</span>
                        <b>
                          {formatSignedScore(activeGroupScoreRow.groupScore)} 分
                        </b>
                      </div>
                    ) : null}
                  </div>
                    ...
                    <div className="group-score-adjust-form">
                      <label className="group-score-adjust-field compact">
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            setGroupScoreDeltaInput(event.target.value)
                          }
                          disabled={
                            !canManageGroupScore || !activeGroupScoreRow
                          }
                        />
                      </label>
                      <label className="group-score-adjust-field grow">
                        ...
                        <textarea
                          ...
                          onChange={(event) =>
                            setGroupScoreRemarkInput(event.target.value)
                          }
                          disabled={
                            !canManageGroupScore || !activeGroupScoreRow
                          }
                  ...
                  {groupScoreRecordsError ? (
                    <div className="group-score-adjust-error">
                      {groupScoreRecordsError}
                    </div>
                  ) : null}
                  ...
                  <div className="group-score-records-block">
                    ...
                    <div className="group-score-records-list">
                      {groupScoreRecordsLoading ? (
                        <div className="group-score-records-empty">
                          加载中...
                        </div>
                      ) : groupScoreRecords.length > 0 ? (
                        groupScoreRecords.map((record) => (
                          <div
                            className="group-score-record-item"
                            key={`group-score-record-${record.id}`}
                          >
                            <div className="group-score-record-item-hd">
                              <span className="group-score-record-group">
                                {record.groupName || `第${record.groupNo}组`}
                              </span>
                              <span
                                className={`group-score-record-delta ${record.scoreDelta >= 0 ? "up" : "down"}`}
                              >
                                {formatSignedScore(record.scoreDelta)}
                              </span>
                            </div>
                            <div className="group-score-record-remark">
                              {record.remark || "—"}
                            </div>
                            <div className="group-score-record-meta">
                              {record.operatorName || "班主任"} ·{" "}
